...
class AreaFilter(object):
    def __init__(self, geojson_geometry, sub_geography_url):
        ...
        geo_query_args = {
            "geometry": ",".join(str(x) for x in self.geo.bounds),
        ...
        self.area_dumper = esridump.EsriDumper(
            sub_geography_url, extra_query_args=geo_query_args
        )
...
class GeoClient(census.core.Client):
    ...
    def geo_tract(self, fields, geojson_geometry, year=None, **kwargs):
        ...
        * year (int) - data year
        ...
        filtered_tracts = AreaFilter(geojson_geometry, GEO_URLS["tracts"][year])
        ...
        for tract, intersection_proportion in filtered_tracts:
            ...
            result = self.get(
                fields,
                HashDict({"for": "tract:{}".format(tract_id), "in": within}),
                year,
                **kwargs,
            )
    ...
    def geo_blockgroup(self, fields, geojson_geometry, year=None, **kwargs):
        ...
        * year (int) - data year
        ...
        filtered_block_groups = AreaFilter(
            geojson_geometry, GEO_URLS["block groups"][year]
        )

        for block_group, intersection_proportion in filtered_block_groups:
            context = {
                "state": block_group["properties"]["STATE"],
            ...
            tract_blockgroups = self.get(
                fields, HashDict({"for": "block group:*", "in": within}), year, **kwargs
            )

            result = [
                result
                for result in tract_blockgroups
    ...
    def _state_place_area(
        self, method, fields, state, place, year=None, return_geometry=False, **kwargs
    ):
        if year is None:
            year = self.default_year

        search_query = "PLACE='{}' AND STATE='{}'".format(place, state)
        place_dumper = esridump.EsriDumper(
            GEO_URLS["incorporated places"][year],
            extra_query_args={"where": search_query, "orderByFields": "OID"},
        )
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    f'Could not find specified place "{place}" in state "{state}"'
                )
    ...
    def geo(
        ...
        **kwargs,
        ...
        if as_acs:
            ...
            for field in acs_fields:
                if self._field_type(field, year) is not int:
                    raise ValueError(
                        "{} is not a variable that can be aggregated your geography".format(
                            field
                        )
                    )
        else:
        ...
        resolutions = {"tract": self.geo_tract, "blockgroup": self.geo_blockgroup}

        try:
            geo_units = resolutions[resolution]
        except KeyError:
            raise ValueError(
                "{} is not a valid resolution. Choose one of {}".format(
                    resolution, resolution.keys()
                )
            )
    ...
    def _aggregate(self, fields, features, year, ignore_missing):
        ...
        for field in fields:
            if field.endswith("E"):
                ...
                e_field = field[:-1] + "E"
                agg = census.math.moe_of_sum(
                    aggregated_features[e_field], aggregated_features[field]
                )
            else:
                raise ValueError(
                    "Don't know how to aggregate this variable {}".format(feature)
                )
...
class GeoBlockClient(GeoClient):
    ...
    def geo_block(self, fields, geojson_geometry, year):
        ...
        * year (int) - data year
        ...
        filtered_blocks = AreaFilter(geojson_geometry, GEO_URLS["blocks"][year])
        ...
        for block, intersection_proportion in filtered_blocks:
            ...
            tract_blocks = self.get(
                fields, HashDict({"for": "block:*", "in": within}), year
            )

            result = [
                result
                for result in tract_blocks
